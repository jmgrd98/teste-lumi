...
const Dashboard = () => {
    ...
    useEffect(() => {
        const fetchFaturas = async (numero_cliente) => {
            try {
                const response = await axios.get(`http://localhost:8080/faturas/cliente/${numero_cliente}`);
    ...
    const energyData = filteredFaturas.map((fatura: Fatura)=> ({
    ...
    const monetaryData = filteredFaturas.map((fatura: Fatura) => ({
        name: fatura.mes_referencia,
        'Total sem GD': fatura.energia_eletrica_valor + fatura.energia_scee_valor + fatura.contrib_ilum_publica,
